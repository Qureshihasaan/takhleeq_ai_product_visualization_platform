...
const FloatingChatbot = () => {
  ...
  const sessionIdRef = useRef(
    localStorage.getItem("aiChat_session_id") || Date.now().toString(),
  );
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        const res = await aiChatbotService.checkHealth();
        if (!mounted) return;
        setBotOnline(!!res && res.status === "ok");
      } catch (err) {
        console.warn("Chatbot health check failed", err);
        if (mounted) setBotOnline(false);
      }
    };
    check();
    return () => {
  ...
  const handleSend = async (e) => {
    ...
    try {
      ...
      // If backend returned a session_id, persist it so conversation can continue
      if (response?.session_id) {
        sessionIdRef.current = response.session_id;
        try {
          localStorage.setItem("aiChat_session_id", response.session_id);
        } catch {}
      }

      const botText =
        response.reply || response.message || "I didn't quite understand that.";
